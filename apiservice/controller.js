...
const fetcharN = async (search = null) => {
  ...
  const token = "&token=7ca717775ec5e7e407b361a789d639ce27dc8224";
  if (search) {
...
const fetchAllIndexesPrices = async (arr) => {
  ...
  const cleanResult = allData.filter((item) => Object.keys(item) > 0);
...
const fetchAllQuotes = async (arr) => new Promise((resolve) => {
  ((async () => {
    console.log(arr);
    let allData = [];
...
const fetchMostActives = () => fetch("https://financialmodelingprep.com/api/v3/actives?apikey=651d720ba0c42b094186aa9906e307b4")
  .then((res) => res.json())
  .catch((err) => err);
