...
const addInterest = async (req, res) => {
  ...
  try {
    ...
    if (data && data.length) {
      ...
      const interestDB = JSON.parse(data[0].interests_arr);
...
const deleteDuplicates = (arr) => arr.filter((v, i, a) => a.findIndex((t) => (t.id === v.id)) === i);
const populateWithTrendingMessages = async (cb) => {
  console.log("executao");
  const fetchStockTwitsTrendingUsers = () => fetch("https://api.stocktwits.com/api/2/streams/trending.json");
...
const fetchStockTwits = (ticker) => fetch(`https://api.stocktwits.com/api/2/streams/symbol/${ticker}.json`)
  .then((res) => res.json());
