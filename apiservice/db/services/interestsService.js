...
module.exports = {
    createInterestTable: () => {
        return db.query("create table if not exists interests (username char(50), interests_arr json, primary key (username), foreign key (username) references users (username))")
    },
    ...
    getUserInterests: (email) => {
        ...
        return new Promise((resolve, reject) => {
            db.query("select * from users inner join interests ON interests.username = users.username where users.email = ?", [email], (err, rows) => {
                ...
                if (!rows) reject(new Error
                    
                    ("wtf, it should give empty array but gave us undefined"));
    ...
    addInterestDB: () => {
        return db.query("insert into interests")
    }
}
